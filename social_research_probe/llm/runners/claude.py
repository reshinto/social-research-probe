...
def _parse_claude_search_body(
    ...
    fallback_citations = [AgenticSearchCitation(url=u) for u in urls]
...
class ClaudeRunner(JsonCliRunner):
    ...
    async def agentic_search(
        ...
        *,
        ...
        The CLI returns its usual JSON envelope ``{"result": "<text>"}``. We
        instruct Claude to emit a JSON block inside ``result`` with
        ``answer`` and ``citations``; if parsing fails, URLs are extracted
        from the free-text reply so the call still yields evidence.
        """
