        ...
        if n["status"] == "new" and n.get("claim_count_change", 0) >= 3:
            signals.append(
                TrendSignal(
                    ...
                    description="New narrative with significant claim backing",
        ...
        if n["status"] == "repeated" and n["strength_signal"] == "weakened":
            signals.append(
                TrendSignal(
                    ...
                    description="Narrative losing strength",
                    narrative_id=n["narrative_id"],
                    score=0.6,
                )
            )
...
def _rising_objections(changes: list[NarrativeChange]) -> list[TrendSignal]:
    ...
    for n in changes:
        if (
            n["cluster_type"] == "objection"
            and n["status"] in ("new", "repeated")
            and (n["status"] == "new" or n["strength_signal"] == "strengthened")
        ):
            signals.append(
                TrendSignal(
                    signal_type="rising_objections",
                    title=f"Rising objection: {n['title']}",
                    description="Objection narrative gaining traction",
                    narrative_id=n["narrative_id"],
                    score=0.75,
                )
            )
...
def _corroboration_trends(changes: list[ClaimChange]) -> list[TrendSignal]:
    ...
    improved = sum(
        1
        for c in repeated
        if c["corroboration_changed"]
        and _is_better_corroboration(c["baseline_corroboration"], c["target_corroboration"])
    )
    worsened = sum(
        1
        for c in repeated
        if c["corroboration_changed"]
        and _is_worse_corroboration(c["baseline_corroboration"], c["target_corroboration"])
    )
