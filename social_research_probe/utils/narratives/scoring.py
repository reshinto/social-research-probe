...
def compute_opportunity_score(claims: list[dict], cluster_type: str) -> float:
    ...
    n = len(claims)
    supported_ratio = sum(1 for c in claims if c.get("corroboration_status") == "supported") / n
...
def compute_risk_score(claims: list[dict], cluster_type: str) -> float:
    ...
    n = len(claims)
    contradiction_ratio = (
        sum(1 for c in claims if c.get("contradiction_status", "none") != "none") / n
    )
    review_ratio = sum(1 for c in claims if c.get("needs_review")) / n
    unsupported_ratio = (
        sum(1 for c in claims if c.get("corroboration_status") in ("pending", "contradicted")) / n
    )
