...
The chart input is `scored_items`: each item has a `scores` dict containing `overall`, `trust`, `trend`, and `opportunity`, plus derived features such as `view_velocity`, `engagement_ratio`, and `age_days`.
