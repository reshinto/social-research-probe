...
| `CLAUDE.md` | Claude Code project instructions. | Provides coding conventions, tool preferences, and project-specific rules for Claude Code sessions. | Update when project conventions or agent workflows change. |
| `runtests.sh` | Test runner script. | Convenience wrapper for running the test suite with common options. | Update when test invocation changes. |
| `.graphifyignore` | Graphify exclusion patterns. | Lists files and directories excluded from knowledge graph indexing. | Update when new generated or irrelevant directories appear. |
...
| `docs/` | Human documentation. | Markdown guides, Mermaid diagram sources, rendered SVGs, and sample images. | Edit when behavior, diagrams, or explanations change. Keep `docs/README.md` linked to every direct `docs/*.md` page. |
...
| `graphify-out/` | Knowledge graph output. | Generated by `graphify update .`. Safe to regenerate. |
| `.claude/` | Local Claude/Codex workspace settings and generated worktrees. | Local-only. Do not rely on it for package behavior. |
