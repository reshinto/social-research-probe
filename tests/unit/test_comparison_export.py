...
def _run_info(run_id: str = "run-a", run_pk: int = 1) -> RunInfo:
    return RunInfo(
        run_pk=run_pk, run_id=run_id, topic="AI", platform="youtube",
        started_at="2024-01-01", finished_at="2024-01-01",
        source_count=5, claim_count=10, narrative_count=3,
    )
...
def _result_with_data() -> ComparisonResult:
    return ComparisonResult(
        ...
        source_changes=[
            SourceChange(source_id=1, platform="youtube", external_id="v1",
                         url="https://yt/v1", title="Video 1", status="new",
                         score_changes={"trust": 0.1, "overall": -0.05},
                         evidence_tier_baseline="", evidence_tier_target="full"),
        ],
        claim_changes=[
            ClaimChange(claim_id="c1", claim_text="Claim A", claim_type="fact",
                        source_url="https://yt/v1", status="repeated",
                        confidence_change=0.1, corroboration_changed=True,
                        baseline_corroboration="pending",
                        target_corroboration="confirmed",
                        review_status_changed=False),
        ],
        narrative_changes=[
            NarrativeChange(narrative_id="n1", title="AI Theme",
                            cluster_type="theme", status="repeated",
                            match_method="exact_id", matched_id="n1",
                            confidence_change=0.2, opportunity_change=0.1,
                            risk_change=-0.05, claim_count_change=2,
                            source_count_change=1, strength_signal="strengthened"),
        ],
        trends=[
            TrendSignal(signal_type="rising_risk", title="Rising",
                        description="desc", narrative_id="n1", score=0.8),
        ],
