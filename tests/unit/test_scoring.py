...
def test_opportunity_bounds():
    s = opportunity_score(market_gap=0.0, monetization_proxy=0.0, feasibility=0.0, novelty=0.0)
    assert s == 0.0
...
def test_norm_z_inner_function_called():
    ...
    s = trend_score(
        z_view_velocity=1.0, z_engagement_ratio=0.5, z_cross_channel_repetition=0.0, age_days=7.0
    )
    assert 0.0 <= s <= 1.0
