...
def test_service_log_sync_records_ok_timing():
    packet: dict = {}
    with sl.service_log_sync("demo", packet=packet, cfg_logs_enabled=False):
        pass
...
def test_service_log_sync_records_error_and_reraises():
    packet: dict = {}
    with (
        pytest.raises(ValueError),
        sl.service_log_sync("demo", packet=packet, cfg_logs_enabled=False),
    ):
...
def test_service_log_sync_emits_only_when_enabled(capsys, monkeypatch):
    monkeypatch.delenv("SRP_LOGS", raising=False)
    packet: dict = {}
    with sl.service_log_sync("quiet", packet=packet, cfg_logs_enabled=False):
        pass
    captured = capsys.readouterr()
    assert captured.err == ""

    with sl.service_log_sync("loud", packet=packet, cfg_logs_enabled=True):
        pass
    captured = capsys.readouterr()
...
def test_service_log_sync_emits_failure_line_when_enabled(capsys, monkeypatch):
    monkeypatch.delenv("SRP_LOGS", raising=False)
    packet: dict = {}
    with (
        pytest.raises(RuntimeError),
        sl.service_log_sync("oops_sync", packet=packet, cfg_logs_enabled=True),
    ):
        raise RuntimeError("kaboom_sync")
    captured = capsys.readouterr()
    assert "oops_sync failed" in captured.err
    assert "kaboom_sync" in captured.err
