...
class TestTextSurrogateServiceContract:
    def test_inherits_base_service(self):
        service = TextSurrogateService()
        assert isinstance(service, BaseService)
        assert service.service_name == "text_surrogate"

    def test_uses_no_external_technologies(self):
        assert TextSurrogateService()._get_technologies() == [None]

    def test_execute_one_wraps_surrogate_in_service_result(self, monkeypatch):
        cfg = type("Cfg", (), {"service_enabled": lambda self, name: True})()
        monkeypatch.setattr("social_research_probe.config.load_active_config", lambda *a, **k: cfg)

        result = asyncio.run(TextSurrogateService().execute_one({"title": "T"}))

        assert result.service_name == "text_surrogate"
        assert result.tech_results[0].tech_name == "text_surrogate_builder"
        assert result.tech_results[0].success is True
        assert result.tech_results[0].output["primary_text"] == "T"

    def test_execute_one_returns_empty_results_when_disabled(self, monkeypatch):
        cfg = type("Cfg", (), {"service_enabled": lambda self, name: False})()
        monkeypatch.setattr("social_research_probe.config.load_active_config", lambda *a, **k: cfg)
