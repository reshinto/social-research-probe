...
def test_suggest_purposes_emits_gap_candidates(tmp_data_dir: Path):
    ...
    drafts = suggest_purposes(count=3)
    assert len(drafts) <= 3
    for d in drafts:
        assert "name" in d
        assert "method" in d
...
def test_apply_pending_all(tmp_data_dir: Path):
    ...
    apply_pending(topic_ids="all", purpose_ids="all")
...
def test_apply_pending_by_ids(tmp_data_dir: Path):
    ...
    apply_pending(topic_ids=[1], purpose_ids="all")
    topics = json.loads((tmp_data_dir / "topics.json").read_text())["topics"]
    assert topics == ["x"]
...
def test_discard_pending_removes_without_applying(tmp_data_dir: Path):
    ...
    discard_pending(topic_ids="all", purpose_ids="all")
    topics = json.loads((tmp_data_dir / "topics.json").read_text())["topics"]
    assert topics == []
    assert _pending(tmp_data_dir)["pending_topic_suggestions"] == []
...
def test_suggest_purposes_breaks_at_count(tmp_data_dir):
    ...
    drafts = suggest_purposes(count=1)
    assert len(drafts) == 1
...
def test_apply_pending_duplicate_purpose_stays_pending(tmp_data_dir):
    ...
    add_purpose(name="trends", method="Track trends", force=False)
    stage_suggestions(
        topic_candidates=[],
        purpose_candidates=[
...
def test_suggest_topics_exhausts_pool(tmp_data_dir):
    ...
    drafts = suggest_topics(count=999)
...
def test_suggest_topics_skips_non_new(tmp_data_dir):
    ...
    from social_research_probe.commands import _TOPIC_SEED_POOL

    add_topics(list(_TOPIC_SEED_POOL), force=True)
    drafts = suggest_topics(count=5)
    assert drafts == []


def test_suggest_purposes_exhausts_pool(tmp_data_dir):
    """Branch 75->80: suggest_purposes loop exhausts all pool candidates."""
    drafts = suggest_purposes(count=999)
    from social_research_probe.commands import _PURPOSE_SEED_POOL
...
def test_suggest_purposes_skips_non_new(tmp_data_dir):
    ...
    drafts = suggest_purposes(count=5)
    assert drafts == []
