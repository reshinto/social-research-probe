...
class TestSummarize:
    ...
    def test_default_now_when_none(self):
        items = [_item()]
        signals = [_signal(upload_date=datetime.now(UTC) - timedelta(days=1))]
        result = summarize(items, signals, [])
...
class TestSummarizeSignals:
    def test_empty_signals_returns_placeholder(self):
        assert summarize_signals([]) == "no signals"

    def test_full_metric_summary(self):
        signals = [_signal(views=1000, velocity=50.0), _signal(views=2000, velocity=150.0)]
