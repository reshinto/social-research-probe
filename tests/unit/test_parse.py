...
from social_research_probe.cli.dsl_parser import (
    DslCommand,
    ParsedApplyPending,
...
def test_update_topics_add():
    result = parse(f'{DslCommand.UPDATE_TOPICS} add:"ai agents"|"robotics"')
...
def test_update_topics_remove():
    result = parse(f'{DslCommand.UPDATE_TOPICS} remove:"ai agents"')
    assert isinstance(result, ParsedUpdateTopics)
...
def test_update_topics_rename():
    result = parse(f'{DslCommand.UPDATE_TOPICS} rename:"old name"->"new name"')
    assert isinstance(result, ParsedUpdateTopics)
...
def test_update_purposes_add_with_method():
    result = parse(f'{DslCommand.UPDATE_PURPOSES} add:"trends"="Track emergence across channels"')
...
def test_apply_pending_all():
    result = parse(f"{DslCommand.APPLY_PENDING_SUGGESTIONS} topics:all purposes:all")
...
def test_apply_pending_ids():
    result = parse(f"{DslCommand.APPLY_PENDING_SUGGESTIONS} topics:1,3 purposes:2,4")
    assert isinstance(result, ParsedApplyPending)
...
def test_discard_pending():
    result = parse(f"{DslCommand.DISCARD_PENDING_SUGGESTIONS} topics:2 purposes:all")
...
def test_run_research_single_topic():
    result = parse(f'{DslCommand.RESEARCH} platform:youtube "ai agents"->trends')
...
def test_run_research_combined_purposes():
    result = parse(f'{DslCommand.RESEARCH} platform:youtube "ai agents"->trends+job-opportunities')
    assert isinstance(result, ParsedRunResearch)
...
def test_run_research_multiple_topics():
    result = parse(
        f'{DslCommand.RESEARCH} platform:youtube "ai agents"->trends;"robotics"->trends+arbitrage'
    )
    assert isinstance(result, ParsedRunResearch)
...
def test_unquoted_topic_raises():
    with pytest.raises(ParseError):
        parse(f"{DslCommand.UPDATE_TOPICS} add:ai agents")
...
def test_rename_trailing_garbage_raises():
    with pytest.raises(ParseError, match="unexpected trailing content"):
        parse(f'{DslCommand.UPDATE_TOPICS} rename:"old"->"new"GARBAGE')


def test_add_purpose_trailing_garbage_raises():
    with pytest.raises(ParseError, match="unexpected trailing content"):
        parse(f'{DslCommand.UPDATE_PURPOSES} add:"trends"="method"GARBAGE')
...
def test_update_topics_invalid_op_raises():
    ...
    with pytest.raises(ParseError):
        parse(f'{DslCommand.UPDATE_TOPICS} mutate:"x"')


def test_update_purposes_remove():
    # _parse_update_purposes remove branch (lines 179-180)
    result = parse(f'{DslCommand.UPDATE_PURPOSES} remove:"old-purpose"')
    from social_research_probe.cli.dsl_parser import ParsedUpdatePurposes
...
def test_update_purposes_rename():
    ...
    result = parse(f'{DslCommand.UPDATE_PURPOSES} rename:"old"->"new"')
    from social_research_probe.cli.dsl_parser import ParsedUpdatePurposes
...
def test_run_research_no_purposes_raises():
    ...
    with pytest.raises(ParseError, match="no purposes"):
        parse(f'{DslCommand.RESEARCH} platform:youtube "AI"->')
...
def test_run_research_invalid_purpose_name_raises():
    ...
    with pytest.raises(ParseError, match="invalid purpose name"):
        parse(f'{DslCommand.RESEARCH} platform:youtube "AI"->has spaces')
...
def test_pending_selectors_space_in_id_list():
    ...
    result = parse(f"{DslCommand.APPLY_PENDING_SUGGESTIONS} topics:1, 2 purposes:all")
    assert isinstance(result, ParsedApplyPending)
